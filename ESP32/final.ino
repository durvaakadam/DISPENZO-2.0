...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    else if (command.equalsIgnoreCase("SEND")) { sendNotification = true; Serial.println("📨 Sending notification..."); Blynk.logEvent("notification", "✅ Confirmation: You have received your monthly ration successfully."); }

else if (command.equalsIgnoreCase("ALERT")) { 
  Serial.println("🚨 Sending notification...");
  Blynk.logEvent("alert_event", " ⚠️ Inventory Alert: Ration stock is low. Immediate refill required.");
}
  ...
  if (tempActive) {
    ...
    delay(500); // read every lc0.5 sec
  }
...
lcd.setCursor(0, 1);
lcd.print("                ");  // 16 spaces for a 16-char row
lcd.setCursor(0, 1);
lcd.printf("FP ID: %-3d", finger.fingerID);
